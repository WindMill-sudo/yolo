...
class ImageAugmentation:
    ...
    def changeImages(self, output_pic, output_xml, function_name, image_name, n, num, s):
        if function_name == "crop":
            ...
            function = self.__addGaussToImage
        elif function_name == "speckle":
            function = self.__addSaltPepperToImage
        ...
        for i in range(1, n + 1):
            ...
            save_image_name = output_pic + '/' + num + '_' + str(s) + '.bmp'
            save_xml_name = output_xml + '/' + num + '_' + str(s) + '.xml'
            print(image_name)
    ...
    def __cropImage(self, img, boxes):
        ...
        if crop_bboxes[0][0] < crop_bboxes[0][2]:
            if crop_bboxes[0][1] < crop_bboxes[0][3]:
    ...
    def __translationImage(self, img, boxes):
        ...
        if shift_bboxes[0][0] < shift_bboxes[0][2]:
            if shift_bboxes[0][1] < shift_bboxes[0][3]:
    ...
    # 5-添加乘性噪声
    def __addSaltPepperToImage(self, img, boxes):
        """
        加入噪声
        :param img: 图像
        :return: img：加入噪声后的图像array,由于输出的像素是在[0,1]之间,所以得乘以255
        """
        newBoxes = copy.deepcopy(boxes)
        newImage = random_noise(img, mode='speckle', clip=True) * 255
        return newImage, newBoxes
    ...
    def __rotateImage(self, img, boxes):
        ...
        if rot_bboxes[0][0] < rot_bboxes[0][2]:
            if rot_bboxes[0][1] < rot_bboxes[0][3]:
    ...
    def __flipImage(self, img, bboxes):
        ...
        if flip_bboxes[0][0] < flip_bboxes[0][2]:
            if flip_bboxes[0][1] < flip_bboxes[0][3]:
                return flip_img, flip_bboxes


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Give old image folder.')
    parser.add_argument('--folder', default='data/inference', help='old image folder')
    parser.add_argument('--output_pic', default='D:\\yolo-defect\\yolov5-6.0\\data\\i1', help='new image folder')
    parser.add_argument('--output_xml', default='D:\\yolo-defect\\yolov5-6.0\data\\a1', help='new xml folder')
    args = parser.parse_args()
    demo = ImageAugmentation()
    folder = os.listdir(args.folder)
    for filename in folder:
        if os.path.splitext(filename)[1] == '.bmp':  # 目录下包含.json的文件
            name = str(args.folder) + '/' + os.path.splitext(filename)[0]
            num = os.path.splitext(filename)[0]
            #demo.changeImages(str(args.output_pic), str(args.output_xml), "crop", name, 1, num, 0)
            demo.changeImages(str(args.output_pic), str(args.output_xml), "tran", name, 1, num, 1)
            #demo.changeImages(str(args.output_pic), str(args.output_xml), "light", name, 1, num, 2)
            #demo.changeImages(str(args.output_pic), str(args.output_xml), "gauss", name, 1, num, 3)
            # demo.changeImages(str(args.output_pic), str(args.output_xml), "speckle", name, 1, num, 4)
            # demo.changeImages(str(args.output_pic), str(args.output_xml), "rotate", name, 1, num, 5)
            demo.changeImages(str(args.output_pic), str(args.output_xml), "flip", name, 1, num, 6)
